(() => {
    ...
    window.addEventListener('load', () => {
        ...
        bg.style.background = `url(${location.origin}/img/background.jpg) center/cover no-repeat fixed`;
    });
    //随机颜色
    ...
    if (sidebarTag) {
        const aList = sidebarTag.getElementsByTagName('a');
    ...
    if (postMetaTag) {
        const aList = postMetaTag.getElementsByTagName('a');
        for (let i = 0; i < aList.length; i++) {
            aList[i].style.color = randomColor();
        }
    }
    ...
    if (mainTag) {
        const aList = mainTag.getElementsByTagName('a');
        for (let i = 0; i < aList.length; i++) {
            aList[i].style.color = randomColor();
        }
    }
    //主页下箭头
    const scrollDownArrow = document.getElementsByClassName('scroll-down-effects')[0];
    if (scrollDownArrow) {
        setInterval(() => {
            scrollDownArrow.style.color = randomColor();
        }, 1500);
    }
    //页脚框架主题
    ...
    const tocSVG = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="none" stroke="#333333" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01"/></svg>';
    function replaceIcon(icon, str) {
    ...
    if (goUp) {
        const goUpIcon = goUp.getElementsByTagName('i')[0];
        replaceIcon(goUpIcon, goUpsvg);
    }
    //侧边栏分类图标
    const cardCategories = document.getElementsByClassName('card-categories')[0];
    if (cardCategories) {
        const cardCategoriesIcon = cardCategories.getElementsByClassName('item-headline')[0].getElementsByTagName('i')[0];
        replaceIcon(cardCategoriesIcon, categoriesSVG);
    }
    //侧边栏标签图标
    const cardTags = document.getElementsByClassName('card-tags')[0];
    if (cardTags) {
        const cardTagsIcon = cardTags.getElementsByClassName('item-headline')[0].getElementsByTagName('i')[0];
        replaceIcon(cardTagsIcon, tagsSVG);
    }
    //侧边栏网站信息图标
    const cardWebinfo = document.getElementsByClassName('card-webinfo')[0];
    if (cardWebinfo) {
        const cardWebinfoIcon = cardWebinfo.getElementsByClassName('item-headline')[0].getElementsByTagName('i')[0];
        replaceIcon(cardWebinfoIcon, infoSVG);
    }
    //主页最近文章设置信息顶置图标
    const articleTitle = document.getElementsByClassName('article-title');
    for (let i = 0; i < articleTitle.length; i++) {
        const articleTitleIcon = articleTitle[i].getElementsByTagName('i')[0];
        if (articleTitleIcon) {
            replaceIcon(articleTitleIcon, null);
        }
    }
    //主页最近文章设置信息创建时间图标
    //主页最近文章设置信息更新时间图标
    ...
    for (let i = 0; i < articleDate.length; i++) {
        const articleCreateDateIcon = articleDate[i].getElementsByTagName('i')[0];
        replaceIcon(articleCreateDateIcon, createSVG);
        const articleUpdateDateIcon = articleDate[i].getElementsByTagName('i')[1];
        replaceIcon(articleUpdateDateIcon, updateSVG);
    }
    //主页最近文章设置信息分类图标
    const articleCategories = document.getElementsByClassName('article-meta');
    for (let i = 0; i < articleCategories.length; i++) {
        const articleCategoriesIcon = articleCategories[i].getElementsByTagName('i')[0];
        replaceIcon(articleCategoriesIcon, categoriesSVG);
    }
    //主页最近文章设置信息标签图标
    ...
    for (let i = 0; i < articleTags.length; i++) {
        ...
        replaceIcon(articleTagsIcon, tagsSVG);
    }
    //文章设置信息创建时间图标
    //文章设置信息更新时间图标
    //文章设置信息分类图标
    ...
    if (metaFirstline && metaSecondline) {
        ...
        const postCreateDateIcon = postDate[0];
        replaceIcon(postCreateDateIcon, createSVG);
        const postUpdateDateIcon = postDate[1];
        replaceIcon(postUpdateDateIcon, updateSVG);
        const postCategoriesIcon = metaFirstline.getElementsByClassName('post-meta-categories')[0].getElementsByTagName('i')[0];
        replaceIcon(postCategoriesIcon, categoriesSVG);
        const postPageview = metaSecondline.getElementsByClassName('post-meta-pv-cv')[0];
        const postPageviewIcon = postPageview.getElementsByTagName('i')[0];
        replaceIcon(postPageviewIcon, pageviewSVG);
    ...
    if (cardToc) {
        ...
        if (itemHeadline) {
            const tocIcon = itemHeadline.getElementsByTagName('i')[0];
            replaceIcon(tocIcon, tocSVG);
        }
    }
})();